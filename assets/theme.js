...
(function(){
  ...
  theme.CartForm = (function() {
    ...
    CartForm.prototype = Object.assign({}, CartForm.prototype, {
      ...
      cartMarkup: function(html) {
		...
		// AMAIS Cart Milestone
		cartMilestone(count, subtotal);
        // AMAIS SideCart SubChecker 
        /*if ($(".shipping_interval")[0]){
               console.log('YES SUBSCRIPTION');
               $("#normalchktbtn").addClass("hideit").removeClass("showit").hide();
               $("#rechargechktbtn").addClass("showit").removeClass("hideit").show();
             } else {
               console.log('NO SUBSCRIPTION');
               $("#normalchktbtn").addClass("showit").removeClass("hideit").show();
               $("#rechargechktbtn").addClass("hideit").removeClass("showit").hide();
             }*/
        // AMAIS SideCart SubChecker ENDS
        this.reInit();
  ...
  theme.announcementBar = (function() {
    ...
    function init() {
      bar = document.getElementById('AnnouncementSliderb');
      if (!bar) {
        return;
      }
      ...
      updatePrice: function(evt) {
        ...
        if (variant) {
          ...
          if (variant.compare_at_price > variant.price) {
			...
			// AMAIS COMMENTED cuz classList Error 
...
function cartMilestone(count, subtotal) {
  ...
  /* AMAIS ADDED FREE SHIPPING BAR */
  $('.cart__checkout').prop('disabled', true);
 setTimeout(function() {
       $('.cart__checkout').prop('disabled', false);
 }, 1500);
  
  var freeshipping = 6000 - subtotal; // Here variable milestone $60
  var freebottle = 9000 - subtotal; // Here variable milestone $90
  ...
  if(freeshipping == 0 && freebottle == 0) {

                  
                  if ( document.documentElement.lang.toLowerCase() === "fr" ) { 
                    $('.dr_fr_ship').html('Yahoo!! Tu as la livraison gratuite et la Bouteille! ');
                    $('.dr_fr_ship_mstone1 .fr_ship_msicon').text('✔️ LIVRAISON GRATUITE');
              $('.dr_fr_ship_mstone2 .fr_ship_msicon').text('✔️ BOUTEILLE GRATUITE');
                  }
                  else { 
                    $('.dr_fr_ship').html('Congrats! you have unlocked free shipping + mug 🥳');
                    $('.dr_fr_ship_mstone1 .fr_ship_msicon').text('✔️ FREE SHIPPING');
              $('.dr_fr_ship_mstone2 .fr_ship_msicon').text('✔️ FREE BOTTLE');
                  }
            ...
            else if(freebottle == 0 && freeshipping > 0 ) {
              
              
                  if ( document.documentElement.lang.toLowerCase() === "fr" ) { 
                    $('.dr_fr_ship').html('Dépense $<span>'+ freeshipping/100 +'</span>  pour avoir la livraison gratuite!');
                    $('.dr_fr_ship_mstone1 .fr_ship_msicon').text('✔️ LIVRAISON GRATUITE');
                  }
                  else { 
                    $('.dr_fr_ship').html('Spend $<span>'+ freeshipping/100 +'</span>  more for free shipping!');
                    $('.dr_fr_ship_mstone1 .fr_ship_msicon').text('✔️ FREE SHIPPING');
                  }
              ...
              $('.dr_fr_shipbar_filler').removeClass('filled').width(10 / 900 * subtotal + '%');
              //$(".dr_fr_ship_chooser").removeClass("dr_fr_ship_choosera");
               
               $('.dr_fr_ship_mstone2 .fr_ship_msicon').text('🎁');
            ...
            else if(freeshipping == 0)
            {
                    ...
                    if ( document.documentElement.lang.toLowerCase() === "fr" ) { 
                      $('.dr_fr_ship').html('Dépense $<span>'+ freebottle/100 +'</span>  pour avoir une Bouteille gratuite!');
                      $('.dr_fr_ship_mstone1 .fr_ship_msicon').text('✔️ LIVRAISON GRATUITE');
                    }
                    else { 
                      $('.dr_fr_ship').html('Spend $<span>'+ freebottle/100 +'</span>  more for free bottle!');
                      $('.dr_fr_ship_mstone1 .fr_ship_msicon').text('✔️ FREE SHIPPING');
                    }
              ...
              $('.dr_fr_ship_mstone2 .fr_ship_msicon').text('🎁');
              $('.dr_fr_shipbar_filler').removeClass('filled').width(10 / 900 * subtotal + '%');
              //$('.dr_fr_shipbar_filler').addClass('filled').width('50%');
            }
            else {
              ...
              $('.dr_fr_ship_mstone1 .fr_ship_msicon').text('🚚');
            }

  /* AMAIS ADDED FREE SHIPPING BAR - END */
  if(subtotal > 9000) {
   setTimeout(function(){
         if ($(".cart__item").hasClass('crt-item7088523444306')){ 
          console.log("FREE BOTTLE EXIST ");
          
         } 
        else {
          console.log("ADD FREE BOTTLE ");
          // Shopify.addItem(40373971353682,1); 
        }  
       
      }, 600); 
  } else {

    if ($(".crt-item7088523444306")[0]){ 
      console.log("FREE BOTTLE EXIST ");
      setTimeout(function(){
        Shopify.removeItem(40373971353682,1);
      }, 100);  
     }
  }     
}
...
function checkFreeBottle(){
  setTimeout(function(){
    if ($(".crt-item7088523444306")[0]){ 
      console.log("FREE BOTTLE EXIST ");
     } 
    else {
      console.log(" FREE BOTTLE DOESN'T EXIST ");
        ...
        $.ajax({
              type: 'POST',
              url: '/cart/add.js',
                data: { quantity: 1, id: 40373971353682 },
                properties: { 'Free-Matcha-Bottle' : "Green" },
              async:false
          });
...
// -------------------------------------------------------------------------------------
// add subscription via ajax
// -------------------------------------------------------------------------------------

Shopify.addSubscription = function(variant_id, plan_id, quantity, callback) {
  var quantity = quantity || 1;
  var params = {
    type: 'POST',
    url: '/cart/add.js',
    data: 'quantity=' + quantity + '&id=' + variant_id + '&selling_plan=' + plan_id,
    dataType: 'json',
    success: function(line_item) {
       document.dispatchEvent(new CustomEvent('cart:build'));
       document.dispatchEvent(new CustomEvent('cart:open'));
...
// Add Ebook
Shopify.addEbook = function(variant_id, quantity, callback) {
  fbq('track', 'SideCartEbook-Amais');
  
   
	if ( $(".ajaxcart__row .grid").hasClass( "ajax_ccgrid_eBook" ) ) {
    console.log("FREE EBOOK ALREADY ADDED");
...
Shopify.addItemUpsell = function(variant_id, quantity, callback) {
  ...
  var params = {
    type: 'POST',
    url: '/cart/add.js',
    data: 'quantity=' + quantity + '&note=cart-upsell-amais-' + variant_id + '&id=' + variant_id,
    dataType: 'json',
    success: function(line_item) {
      if ((typeof callback) === 'function') {
        callback(line_item);
...
Shopify.addCollagen = function(variant_id, quantity, callback) {
  ...
  //return CartDrawer;
  setTimeout(function(){  $('#carttopr').click(); }, 700); 
  
};
// Add addTumbler

Shopify.addTumbler = function(variant_id, quantity, callback) {
  var quantity = quantity || 1;
  // Set a cookie
	Cookies.set('dr_fr_ship_chooser_color', variant_id);
	if ( $(".ajaxcart__row .grid").hasClass( "ajax_ccgrid_Cadeau" ) ) {
...
//Remove Free Tumblr
Shopify.removeTumbler = function(variant_id, callback) {
  var params = {
    type: 'POST',
    url: '/cart/change.js',
    data:  'quantity=0&id='+variant_id,
...
document.addEventListener('rebuy:cart.ready', (event) => { 
  document.querySelector('#carttopr').classList.add('rebuy-loaded')
});
...
// AMAIS CASES For recharge widget
/* $(window).on("load", function () {
  var current_var = $(".def-var-sec-0").data("def-variant");
  console.log(current_var);
  setTimeout(function() {
    rechargeVariants(current_var);
  }, 2000);
}); */
...
//     //$( ".rc_block__type__onetime:first" ).click();
//     $(".rc_radio__onetime:first").prop("checked", true).click();
// 	  $("#rc_container").hide();
//         $(".product-block--price").show();  
//   }
//   function rcWidgetOn() {
//     $("#rc_container").show();
//     $(".product-block--price").hide();
//   }
// 	switch (varx) {
//         //Collagen
//     case 39617971716178:
